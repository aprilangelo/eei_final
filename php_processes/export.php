...
$query = "SELECT 'Open' as ticket_status, COUNT(ticket_id) AS count FROM ticket_t WHERE ticket_status < '7' AND (ticket_t.date_prepared BETWEEN '$startdate' AND '$enddate')
UNION SELECT 'Closed' ticket_status, COUNT(ticket_id) AS count FROM ticket_t WHERE ticket_status > '6' and (ticket_t.date_prepared BETWEEN '$startdate' AND '$enddate')
";
$result = mysqli_query($db,$query);
$row2=$row2+1;
...
$query = "SELECT t.ticket_category, count(t.ticket_id) as 'Ticket Count' FROM ticket_t t WHERE t.ticket_category is not null AND t.ticket_category!='' and (t.date_prepared BETWEEN '$startdate' AND '$enddate')
group by t.ticket_category UNION SELECT 'Unassigned', count(t.ticket_id) as 'Ticket Count' FROM ticket_t t WHERE t.ticket_category is null AND (t.date_prepared BETWEEN '$startdate' AND '$enddate')";
...
#tickets per month
$query = "SELECT MONTHNAME(t.date_prepared) as Month, count(t.ticket_id) as 'Ticket Count' FROM ticket_t t WHERE (t.date_prepared BETWEEN '$startdate' AND '$enddate') group by MONTHNAME(t.date_prepared)";
$result = mysqli_query($db,$query);
$row=2;
while($row_data = mysqli_fetch_assoc($result)){
